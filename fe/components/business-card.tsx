...
export default function BusinessCard({ business, onView }: { business: Business; onView?: (id: string) => void }) {
  return (
    <div className={`group bg-white border rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-green-100/50 hover:-translate-y-1 ${business.featured ? 'border-green-300 ring-1 ring-green-100' : 'border-green-100'}`}>
      {/* Cover Image */}
      <div className="relative h-36 overflow-hidden bg-gradient-to-br from-green-100 to-emerald-50">
        ...
        />
        {/* {business.featured && (
          <span className="absolute top-3 right-3 px-2.5 py-1 rounded-lg text-xs font-bold bg-green-600 text-white">
            Featured
          </span>
        )} */}
      </div>
      ...
      <div className="px-5 -mt-8 relative">
        <div className="flex items-end gap-3 mb-3">
          ...
          <div className="pb-1">
            <h3 className="font-bold text-lg text-gray-900 group-hover:text-green-700 transition-colors leading-tight">
              {business.name}
            </h3>
      ...
      <div className="px-5 pb-5 space-y-3">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-0.5">
            {[1, 2, 3, 4, 5].map((star) => (
              <svg
                key={star}
                className={`w-4 h-4 ${star <= Math.round(business.rating) ? 'text-amber-400' : 'text-gray-200'}`}
                fill="currentColor"
                viewBox="0 0 20 20"
              >
        ...
        {business.services.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {business.services.slice(0, 3).map((service) => (
              <span key={service} className="px-2 py-0.5 rounded-md bg-green-50 text-xs font-medium text-green-700">
                {service}
              </span>
            ))}
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-green-600 text-white text-sm font-bold hover:bg-green-700 transition-all shadow-sm shadow-green-200"
        >
          View Full Profile
        </button>
      </div>
